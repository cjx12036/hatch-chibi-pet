...
STYLE_PRESETS = {
    ...
    # Modified for hatch-chibi-pet: add a dedicated human-photo chibi preset.
    "chibi": (
        "Cute polished chibi sticker character, about 2.5 heads tall, with an "
        "oversized head, compact rounded body, short limbs, large readable facial "
        "features, clean dark outline, flat colors, and minimal soft cel shading."
    ),
    "flat-vector": (
...
def sentence(value: str) -> str:
    ...
    if not value:
        return value
    # Modified for hatch-chibi-pet: preserve Chinese sentence punctuation.
    if value[-1] not in ".!?。！？":
        value += "."
    return value
